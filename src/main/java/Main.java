...
//        run("run0", data1, KEY_128_1, expectedResult1);
        ...
        run("example_1_output.txt", data2, KEY_128_2, expectedResult2);
        ...
        run("example_2_output.txt", data3, KEY_192_3, expectedResult3);
        ...
        run("example_3_output.txt", data4, KEY_256_4, expectedResult4);

    }

    private static void run(String filename, String data, String key, String expectedResult) {
        String result = AESCryptor.cipher(filename, data, key);
        System.out.println(result);
        if(!result.equals(expectedResult))
            throw new RuntimeException("Result differend from expected result");
        String result2 = AESCryptor.decipher(filename, result, key);
